...
export function AppShell({ children }: Props) {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 md:flex-row">
      {/* ── Desktop sidebar ─────────────────────────────────────────── */}
      <aside className="hidden md:flex flex-col w-56 border-r border-slate-800 bg-slate-900 shrink-0">
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <nav className="md:hidden fixed bottom-0 inset-x-0 flex border-t border-slate-800 bg-slate-900 z-50">
          {TABS.map(({ href, label, icon: Icon }) => {
            const active = pathname.startsWith(href);
            return (
              <Link
