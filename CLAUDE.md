...
- **Graph schema (`/public/venue/graph.json`).** 40 nodes: 12 outer corridor ring (`c-o-0`…`c-o-11`), 12 inner corridor ring (`c-i-0`…`c-i-11`), 1 user seat (`n-seat` at ~560,215), 15 POI nodes. All coords in 1000×600 viewBox. Parsed once via memoized `getGraph()` in `/lib/routing/index.ts`.
- **Routing weight formula:** `w = baseDistance × (1 + CROWD_PENALTY_MAX × max(density[from], density[to]))`. CROWD_PENALTY_MAX = 2.0. Higher density → higher edge cost → path avoids crowded areas.
- **User seat convention:** The user's fixed location is `n-seat` (node ID). All routes originate from this node. Coords: x=560, y=215.
- **ETA formula:** `etaSec = round(totalWeight / WALKING_SPEED_SVG_PER_SEC / ROUTING_ETA_ROUND_TO_SEC) × ROUTING_ETA_ROUND_TO_SEC`. WALKING_SPEED_SVG_PER_SEC=12, ROUTING_ETA_ROUND_TO_SEC=5.
- **Route overlay:** `<RouteOverlay path={string[]} graph={VenueGraph} />` renders a `motion.polyline` with `pathLength` 0→1 animation. Accepts node ID arrays. Rendered as `children` inside `<VenueHeatmap>`.
- **MockSimulationBoot:** `<MockSimulationBoot />` is mounted in `app/(app)/layout.tsx`. It calls `startSimulation(pois)` once on mount (client-side) when `NEXT_PUBLIC_MOCK_MODE=true`. This auto-advances crowd density every 3s so the map is always live without needing admin panel interaction.
- **Concierge request/response contract.** Request: `{messages: ConciergeMessage[], userLocation: {nodeId: string}}`. Response: `{reply: string, recommendation: ConciergeRecommendation | null, action: "navigate"|"order"|"info"}`. Full schemas in `/lib/schemas/concierge.ts`. Assistant messages in the `messages[]` array are stored as JSON strings (the full `ConciergeResponse` serialized), so Claude sees its own prior structured responses in context.
- **`estimateWaitSec(density, poiType)`** is the single source of truth for wait-time estimates. Lives in `/lib/mock/waitTime.ts`. Used by both `/concierge` (via the system prompt builder) and `/map` (POI sheet). food: 0→600s, restroom: 0→240s, others: 0.
- **Order state machine.** `placed → preparing → ready → collected`. Transitions are one-way and driven by the admin panel. State lives in `globalThis.__stadiumiq_order_store` (server-side singleton, mirrors crowd store pattern). `advance()` is a no-op if already `collected`.
- **`X-User-Id` header convention.** Client passes a random UUID from localStorage as `X-User-Id` header on every `/api/order` request. Never in the request body. Server reads `request.headers.get("X-User-Id")`. Generated/stored by `getOrCreateUserId()` in `/lib/user/identity.ts`.
- **Order store + data abstraction.** `/lib/mock/orderStore.ts` mirrors `/lib/mock/store.ts`: `globalThis` singleton, `Map<id, Order>`, pub/sub listeners. `/lib/data/orders.ts` mirrors `/lib/data/crowd.ts`: abstraction layer, mock branch polls `/api/order` every 500ms. Components/hooks never import from `/lib/mock/` directly.
- **Deep-link convention.** Navigating to `/map?nav=<poiId>` auto-activates routing from `n-seat` to that POI. The map page reads `useSearchParams()` on mount and sets `navTarget`. "Clear route" calls `router.replace('/map')` to remove the param. The concierge's "Take me there" button uses this convention.
